...
unsigned char ModBus::construct_F15()
{
...
unsigned char ModBus::construct_F16()
{
...
void ModBus::waiting_for_turnaround()
{
  if (millis.hasExpired(polling))
        state = IDLE;
}

// get the serial data from the buffer
void ModBus::waiting_for_reply()
{
    if (ModbusPort->bytesAvailable()) // is there something to check?
    {
        unsigned char overflowFlag = 0;
        buffer = 0;
        while (ModbusPort->bytesAvailable())
        {
            ...
            else
            {
                ...
                QByteArray data = ModbusPort->readAll();
                for(int i=0;i<data.size();i++) frame[buffer++] = data[i];
            }
            ...
            QThread::msleep(T1_5); // inter character time out
        }
    ...
    else if (millis.hasExpired(timeout)) // check timeout
    {
        processError();
...
void ModBus::processError()
{
  ...
  millis.restart(); // start the turnaround delay
}

void ModBus::processSuccess()
{
...
void ModBus::modbus_configure(QSerialPort*  SerialPort,
                                            long baud,
                                            long _timeout,
                                            long _polling,
                                            ...
                                            unsigned int* _register_array)
{
    ...
    ModbusPort->open(QIODevice::ReadWrite);
}

void ModBus::modbus_construct(Packet *_packet,
                                            unsigned char id,
...
unsigned int ModBus::calculateCRC(unsigned char bufferSize)
{
...
void ModBus::sendPacket(unsigned char bufferSize)
{
    for (unsigned char i = 0; i < bufferSize; i++)
        ModbusPort->write((const char *)&(frame[i]));

    ModbusPort->flush();

    QThread::usleep(frameDelay);

    millis.restart(); // start the timeout delay
}
