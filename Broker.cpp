...
void Broker::config()
{
    RdKafka::Conf* _producer_conf = RdKafka::Conf::create(RdKafka::Conf::CONF_GLOBAL);
 ...
 */
    ...
    if (_producer_conf->set("bootstrap.servers", "localhost:9092", err_msg) != RdKafka::Conf::CONF_OK) {
        SetConfigError(true, err_msg);
        return;
    }
...
void AcornDeliveryReportCb::dr_cb(RdKafka::Message& message)
{
    // Callback on every message delivery -> triggered by producer->poll(n>0);
    if (message.err()) {
        std::string error_message = "% Message delivery failed: " + message.errstr();
        std::cerr << error_message << std::endl;
    }
}

void AcornEventCb::event_cb(RdKafka::Event& event)
{
    // Callback called on every Kafka event -> triggered by
  // consumer/producer->poll(n>0);
